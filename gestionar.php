...
try {
    ...
    if (!gestion_can_view($item, $roles, $emailRaw)) throw new RuntimeException('Tu cuenta no tiene acceso a este reporte.');

    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (!hash_equals($csrfToken, (string) ($_POST['csrf_token'] ?? ''))) throw new RuntimeException('La sesión del formulario expiró. Actualiza la página e inténtalo nuevamente.');
        if (!gestion_can_edit($item, $roles)) throw new RuntimeException('Tu cuenta puede consultar este reporte, pero no modificarlo.');
        ...
        if ($action === 'tomar_reporte') {
            ...
            if (!in_array($status, $allowedStatuses, true)) throw new RuntimeException('Selecciona un estatus válido.');
            if (mb_strlen($seguimiento) > 4000 || mb_strlen($solucion) > 4000) throw new RuntimeException('Seguimiento o solución exceden el máximo permitido.');
            if (in_array($status, ['Solucionado', 'Cerrado'], true) && $solucion === '') throw new RuntimeException('Para cerrar o marcar como solucionado debes registrar la solución.');

            $update = ['Estatus' => $status, 'Seguimiento' => $seguimiento, 'Solucion' => $solucion];
            if (gestion_val($item, ['ResponsableCorreo', 'Responsable Correo']) === '') {
    ...
    if (!empty($item['Attachments'])) $attachments = reportes_item_attachments($itemId);
} catch (Throwable $ex) {
...
$isCapillas = strcasecmp($area, 'Capillas') === 0;
?>
<!doctype html>
<html lang="es-MX">
<head>
  ...
  <main class="shell reportes-main">
    ...
    <?php else: ?>
      ...
      <section class="create-card">
        ...
        <?php else: ?>
          <p>Reporte de Capillas: la descripción contiene la información necesaria para su atención.</p>
        <?php endif; ?>
        ...
        <section class="create-card">
          ...
          <?php if ($responsibleEmail === ''): ?>
            <form method="post" class="form-actions" style="margin-top:18px">
              <input type="hidden" name="id" value="<?= $itemId ?>"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="action" value="tomar_reporte"><button type="submit">Asignarme este reporte</button>
            </form>
          <?php else: ?><div class="report-summary"><span>Responsable: <?= htmlspecialchars($responsible !== '' ? $responsible : $responsibleEmail, ENT_QUOTES, 'UTF-8') ?></span></div><?php endif; ?>

          <form class="report-form" method="post">
            <input type="hidden" name="id" value="<?= $itemId ?>"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="action" value="actualizar_reporte">
      ...
      <?php else: ?>
        <section class="create-card">
          ...
          <div class="reportes-meta"><div><span>Responsable</span><strong><?= htmlspecialchars($responsible !== '' ? $responsible : 'Sin asignar', ENT_QUOTES, 'UTF-8') ?></strong></div><div><span>Fecha de cierre</span><strong><?= htmlspecialchars($closedAt !== '' ? $closedAt : '—', ENT_QUOTES, 'UTF-8') ?></strong></div><div><span>Cerrado por</span><strong><?= htmlspecialchars($closedBy !== '' ? $closedBy : '—', ENT_QUOTES, 'UTF-8') ?></strong></div></div>
